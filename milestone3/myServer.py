...
def get_input(conn, currentPlayer):
    try:
        ...
        board[x][y] = currentPlayer
        send_board_to_all()  # Send board updates to both players
        time.sleep(0.5)
        send_msg(conn, "Matrix")
        time.sleep(0.5)
        send_msg(conn, str(board))
...
def list_connected_players():
    print("Available Players:")
    for i, addr in enumerate(playerAddr, start=1):
        print(f"Player {i} - {addr[0]}:{addr[1]}")
